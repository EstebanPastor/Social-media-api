...
userController.put("/updateUser/:userId", verifyToken, async (req, res) => {
  if (req.params.userId === req.user.id) {
    ...
    return res
      .status(403)
      .json({ msg: "You can only change your own profile" });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (currentUserId === otherUserId) {
        return res.status(500).json({ msg: "You can't unfollow yourself" });
      }
      const currentUser = await User.findById(currentUserId);
      const otherUser = await User.findById(otherUserId);

      // if we don't follow an use, we want to unfollow him, otherwise we follow him
